...
Original file is located at
    https://colab.research.google.com/drive/1Kuw5pQ33i6Sox7SYLx8gPKdnbr2Rip-M
"""
...
#se define cintura
cintura = datos_mexico[:,1]

print(f"Media de cintura: {np.mean(cintura):.2f} cm")
print(f"Mediana de cintura: {np.median(cintura): .2f} cm")
print(f"Desviación estándar de cintura: {np.std(cintura):.2f} metros")
print(f"cintura mínima: {np.min(cintura):.2f} metros")
print(f"cintura máxima: {np.max(cintura):.2f} metros")
print(f"Percentil 25 de cintura: {np.percentile(cintura, 25):.2f} metros")
print(f"Percentil 75 de cintura: {np.percentile(cintura, 75):.2f} metros")

# Visualización de cintura
plt.figure(figsize=(10, 6))
plt.hist(cintura, bins=20, color='orange', alpha=0.7)
plt.title('Distribución de Medidas de Cintura en México')
plt.xlabel('Cintura (cm)')
plt.ylabel('Frecuencia')
plt.show()

altura = datos_mexico[:,0]

print(f"Media de altura: {np.mean(altura):.2f} m")
print(f"Mediana de altura: {np.median(altura): .2f} m")
print(f"Desviación estándar de altura: {np.std(altura):.2f} metros")
print(f"Altura mínima: {np.min(altura):.2f} metros")
print(f"Altura máxima: {np.max(altura):.2f} metros")
print(f"Percentil 25 de altura: {np.percentile(altura, 25):.2f} metros")
print(f"Percentil 75 de altura: {np.percentile(altura, 75):.2f} metros")

# Visualización de altura
plt.figure(figsize=(10, 6))
plt.hist(altura, bins=20, color='purple', alpha=0.7)#purple
plt.title('Distribución de Medidas de Alturas en México')
plt.xlabel('Altura (m)')
plt.ylabel('Frecuencia')
plt.show()
